...
describe('Retirement Calculator Validation (Negative Testing)', () => {
    ...
    it('should show specific error when retirement age is less than or equal to current age', async () => {
        await CalculatorPage.setInputValue(CalculatorPage.inputCurrentAge, CURRENT_AGE_FOR_INVALID_TEST);
        await CalculatorPage.setInputValue(CalculatorPage.inputRetirementAge, RETIREMENT_AGE_LESS_THAN_CURRENT);
